...
def grid_sample(input, x_shape, y_shape):
    ...
    if rem_dim:
        ...
        grid = torch.stack((meshy, meshx), 2).unsqueeze(0).to(device='cuda:0').half()
...
class Decoder(nn.Module):
    ...
    def forward(self, f16, f8, f4, hidden_state, memory_readout, image=None, h_out=True):
        ...
        full_res_g4 = g4 #F.interpolate(g4.squeeze(0), size=(x_shape, y_shape), mode='bilinear', align_corners=False).unsqueeze(0)
